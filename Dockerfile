...
#
# -- HOWEVER THE CUDA VERSION OUTSIDE ON YOUR VMs MUST MATCH THE VERSION (v12.3) HERE *EXACTLY*!
#
#    You can compare with `nvidia-smi` outside the container vs `nvcc --version` inside.
#    Find alternate [nvidia/kaldi] images here:
...
FROM nvcr.io/nvidia/kaldi:23.11-py3
...
ENV OPENFST=${KALDI_TOOLS}/openfst-1.7.2
ENV KLIBS="\
$OPENFST/lib/libfst.so \
